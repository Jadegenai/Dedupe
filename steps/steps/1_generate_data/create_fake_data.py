...
def generate_duplicates(df, num_rows=5):
    ...
    for _ in tqdm(range(num_rows)):
        ...
        if "Address Change" in selected_changes:
        ...
        if column == "Email":
            random_float = random.random()
            names = new_row["Name"].to_list()[0].split(" ")
            new_email = generate_name_related_or_random_email(names)
...
def generate_distractors(df:pd.core.frame.DataFrame, num_rows:int):
    new_rows = []
    for _ in tqdm(range(num_rows)):
        column = ""
        new_row = df.sample(n=1)
        for i in range(random.randint(1,len(df.columns))):
            column = random.choice(df.columns)
            if column == "Name":
                new_row[column] = fake.name()
            elif column == "Street Address":
                new_row[column] = fake.street_address()
            elif column == "City":
                new_row[column] = fake.city()
            elif column == "State":
                ...
                new_row[column] = fake.phone_number()
            elif column == "Tax ID/SSN":
                new_row[column] = fake.ssn() if random.random() < 0.7 else generate_fake_tax_id()
            elif column == "Email":
                new_row[column] = fake.email()
...
df = pd.read_csv(r"steps\1_generate_data\data\fake_deduplication_data_base.csv")[:1000]
...
df = pd.concat([df,generate_distractors(df,1000)],ignore_index=True)

df.to_csv(r"steps\1_generate_data\data\fake_deduplication_data_v2_small.csv",index=False)